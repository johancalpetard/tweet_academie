...
class Messagerie
{
    ...
    {
        $query = $this->bdd->prepare('select * from messages inner join user_info where messages.from_id = user_info.id_user AND from_id=? && to_id=? OR from_id=? && to_id=? group by message_content');
        $query->execute(array(
            $from,
            $to,
            $to,
